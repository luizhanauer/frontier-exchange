...
describe("CurrencyCalculator Domain", () => {
  ...
  describe("Conversão Simples", () => {
    it("deve converter Dólar para Real usando taxa oficial", () => {
      ...
      const result = CurrencyCalculator.convert(
        100,
        mockRates.USD!,
        mockRates.BRL!,
      );
    ...
    it("deve incluir os símbolos das moedas no resumo do WhatsApp", () => {
      const items = [
        { id: "1", name: "Teste", price: 10, currencyCode: "USD" },
      ];
      const summary = ShoppingListCalculator.generateWhatsAppSummary(
        items,
        mockRates,
      );
      const decodedSummary = decodeURIComponent(summary);

      expect(decodedSummary).toContain("US$ 10.00");
      expect(decodedSummary).toContain("Total em Real: R$ 50.00");
    });
  });
  ...
  describe("ShoppingListCalculator", () => {
    ...
    it("deve calcular o total da lista convertido para BRL corretamente", () => {
      const total = ShoppingListCalculator.calculateTotalInCurrency(
        items,
        mockRates,
        "BRL",
      );
